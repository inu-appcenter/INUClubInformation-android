...
public class Image4 extends Fragment {
    ...
    public View onCreateView(LayoutInflater inflater, final ViewGroup ImageContainer, Bundle savedInstanceState){
        LinearLayout layout = (LinearLayout) inflater.inflate(R.layout.apply_image4, ImageContainer, false);

        final ImageView image4 = (ImageView) layout.findViewById(R.id.image4);
        ...
        call.enqueue(new Callback<ArrayList<JsonObject>>() {
            ...
            public void onResponse(Call<ArrayList<JsonObject>> call, Response<ArrayList<JsonObject>> response) {
                imageLocation = response.body().get(0).getAsJsonObject().get("image4").toString().replace("\"", "");
                if(imageLocation.length() < 5)
                {
                    image4.setImageResource(R.drawable.group_6);
                }
                else{
                    String url = NetworkController.getURL() + imageLocation;
                    Glide.with(Image4.this).load(url).into(image4);
                }
            }
